...
static void
proxy_manager_signal_handler(int32_t signum, siginfo_t *info, void *ctx)
{
}

static void
proxy_worker_register(proxy_worker_t *worker)
{
...
static void
proxy_worker_deregister(proxy_worker_t *worker)
{
    proxy_manager_t *manager;
...
static void
proxy_worker_finished(proxy_worker_t *worker)
{
    proxy_manager_t *manager;
...
proxy_manager_main(void *arg)
{
    ...
    while (true) {
        while (!list_empty(&manager->finished)) {
            worker = list_first_entry(&manager->finished, proxy_worker_t, list);
            list_del_init(&worker->list);
...
static int32_t
proxy_manager_init(proxy_manager_t *manager)
{
    int32_t err;
...
static void
proxy_manager_destroy(proxy_manager_t *manager)
{
...
static int32_t
proxy_manager_setup_signals(struct sigaction *old)
{
...
static void
proxy_manager_restore_signals(struct sigaction *action)
{
    proxy_signal_set(SIGCONT, action, NULL);
}

static void
proxy_manager_terminate(proxy_manager_t *manager)
{
    proxy_worker_t *worker;
...
int32_t
proxy_manager_run(proxy_manager_t *manager, proxy_manager_main_t main)
{
...
void
proxy_manager_shutdown(proxy_manager_t *manager)
{
    proxy_mutex_lock(&manager->mutex);
...
int32_t
proxy_manager_launch(proxy_manager_t *manager, proxy_worker_t *worker,
                     proxy_worker_main_t main, proxy_worker_destroy_t destroy)
{
    int32_t err;
